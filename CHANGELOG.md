...
### Release: Skills Architecture — Canonical Skill Format for All IDEs

**Type:** Minor Version Bump (new architecture + build pipeline + test coverage)

#### Added

- **Canonical Skill Format** (`skills/<id>/SKILL.md`) — Single source of truth for all IDE configs
  - YAML frontmatter schema: `id`, `name`, `description`, `version`, `tags`, `applies_to`, `author`, `tools`, `globs`, `alwaysApply`
  - 11 canonical skills migrated from `skills/shared-prompts/`: guardrails-enforcer, commit-validator, env-separator, scope-validator, production-first, three-strikes, error-recovery, 3d-game-dev, four-laws, halt-conditions, vibe-coding

- **`scripts/build_skills.py`** — Build pipeline generating all native IDE formats from canonical sources
  - `scripts/skill_lib/parser.py` — Parse YAML frontmatter, validate schema
  - `scripts/skill_lib/generator.py` — Generate Claude JSON, Cursor MD, OpenCode/OpenClaw SKILL.md
  - `scripts/skill_lib/assembler.py` — Assemble monolithic Copilot + Windsurf files
  - `scripts/skill_lib/rewriter.py` — Rewrite cross-skill references per platform
  - CLI: `--check` (drift detection), `--dry-run`, `--platform`, `--skill`

- **Plugin Manifests** — Lightweight IDE-specific plugin directories referencing `./skills/`
  - `.claude-plugin/plugin.json`
  - `.cursor-plugin/plugin.json`
  - `.codex-plugin/plugin.json`
  - `.gemini-extension/gemini-extension.json`

- **Test Coverage** (`tests/scripts/`) — 44 pytest tests for the build pipeline
  - `test_skill_parser.py` — Frontmatter parsing and schema validation
  - `test_generators.py` — Claude JSON, Cursor MD, OpenCode/OpenClaw generation
  - `test_rewriter.py` — Cross-reference rewriting per platform
  - `test_assembler.py` — Monolithic file assembly and deterministic ordering

- **CI/CD Drift Check** (`.github/workflows/skill-build-check.yml`)
  - Runs on push/PR when `skills/**`, `scripts/build_skills.py`, `scripts/skill_lib/**`, or `tests/scripts/**` change
  - `test` job: `pytest tests/scripts/ -v`
  - `drift-check` job: `python scripts/build_skills.py --check` (fails PR on stale generated files)

- **Top-Level Agent Bootstrappers**
...
- **`scripts/setup_agents.py`** — Added `ensure_skill_exists()` wrapper
  - Delegates to `build_skills.py` when generated native files are missing
  - Full backward compatibility with `--install`, `--clone`, `--install-skill`, `--list-skills`

- **Generated Files** — All native IDE files now carry `GENERATED by build_skills.py — do not edit directly` headers
  - `.claude/skills/*.json`
  - `.cursor/rules/*.md`
...
- **Documentation Restructure**
  - `docs/AGENTS_AND_SKILLS_SETUP.md` — Split from 521 lines to ~160 lines (quick start only)
  - `docs/SKILL_REGISTRY.md` — Per-skill details and customization guide
  - `docs/SKILLS_ARCHITECTURE.md` — Build script usage, CI/CD integration, batch workflows
  - `CLAUDE.md` (root) — Updated with skills pointer and plugin architecture section

- **`README.md`** — Updated for v3.2.0
  - Added "Skills Architecture" capability row
  - Replaced "AI Tool Integration" with canonical-to-native format table (6 IDE layers)
  - Updated project structure to show `skills/<id>/`, `scripts/build_skills.py`, plugin manifests
  - Replaced `skills/shared-prompts/` links with `skills/<id>/SKILL.md`
