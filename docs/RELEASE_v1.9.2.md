...
After deploying v1.9.1, accessing the Web UI at `http://100.96.49.42:8093` returned:
```json
...
2. Deploy to AI01: Transfer image and restart container
3. Verify: Access `http://100.96.49.42:8093` without API key
...
```bash
curl http://100.96.49.42:8093/
curl http://100.96.49.42:8093/index.html
curl http://100.96.49.42:8093/static/style.css
```

### API endpoints (still require auth)
```bash
# Should fail with 401
curl http://100.96.49.42:8093/mcp/v1/sse

# Should succeed with valid key
curl -H "Authorization: Bearer $MCP_API_KEY" http://100.96.49.42:8092/mcp/v1/sse
```
